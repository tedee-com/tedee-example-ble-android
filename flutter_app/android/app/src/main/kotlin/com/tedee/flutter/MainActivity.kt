...
class MainActivity : FlutterActivity(), ILockConnectionListener {
    ...
    override fun configureFlutterEngine(@NonNull flutterEngine: FlutterEngine) {
        ...
        methodChannel?.setMethodCallHandler { call, result ->
            when (call.method) {
                ...
                "requestStateSync" -> {
                    // Flutter is ready - request state sync from service if running
                    if (isBackgroundServiceRunning()) {
                        Timber.d("📡 Flutter requested state sync")
    ...
    override fun onResume() {
        ...
        // No delay needed - Flutter doesn't request state sync on startup anymore,
        // so broadcasts will only arrive when service naturally sends them (on state changes)
